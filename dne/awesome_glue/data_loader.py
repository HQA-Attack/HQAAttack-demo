from dne.awesome_glue.task_specs import TASK_SPECS
from dne.allennlpx.data.dataset_readers.spacy_tsv import SpacyTSVReader
from dne.allennlpx.data.dataset_readers.berty_tsv import BertyTSVReader
from allennlp.data.vocabulary import Vocabulary
from dne.allennlpx.interpret.attackers.attacker import DEFAULT_IGNORE_TOKENS
from copy import copy
from dne.luna import auto_create
import json
...
def load_data(task_id: str, tokenizer: str):
    ...
    def __load_data():
        train_data = reader.read(f'dne/{spec["path"]}/train.tsv')
        dev_data = reader.read(f'dne/{spec["path"]}/dev.tsv')
        test_data = reader.read(f'dne/{spec["path"]}/test.tsv')
        _MIN_COUNT = 3
